...
struct HostDataPacket {
    HostDataPacket(unsigned size, void* in_data, const std::string& stream_name_, std::vector<int> dimensions_, int elem_size_)
        : stream_name(stream_name_), dimensions(dimensions_), elem_size(elem_size_) {
        int frameSize = size;
        ...
        if(metadata.isValid()) {
            ...
            // printf("Stream: %s (size: %d, frameSize: %d), metadata packet valid: w:%d, h:%d, t:%d, %6.3f\n", stream_name_.c_str(),size, frameSize,
            // metadata.spec.width,metadata.spec.height, metadata.spec.type, metadata.getTimestamp());
            // set opt_metadata
