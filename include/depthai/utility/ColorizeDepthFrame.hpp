 ...
 * @param minDepth Lower depth bound in millimeters of the range to map to 0 (default: 300 mm).
...
ImgFrame colorizeDepthFrame(
    const ImgFrame& frame, float minDepth = 300.0f, float maxDepth = 12000.0f, cv::ColormapTypes colormap = cv::COLORMAP_JET, bool useLog = true);
...
cv::Mat colorizeDepthFrame(
    const cv::Mat& frame, float minDepth = 300.0f, float maxDepth = 12000.0f, cv::ColormapTypes colormap = cv::COLORMAP_JET, bool useLog = true);
