...
class CNNHostPipeline : public HostPipeline {
   private:
   ...
   public:
    CNNHostPipeline(const std::vector<TensorInfo>& tensors_info) : _tensors_info(tensors_info) {}
    virtual ~CNNHostPipeline() {}

    std::tuple<std::list<std::shared_ptr<NNetPacket>>, std::list<std::shared_ptr<HostDataPacket>>> getAvailableNNetAndDataPackets(bool blocking = false);
};
