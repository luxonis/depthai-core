...
/**
 * @brief NeuralDepth node. Compute stereo disparity and depth from left-right image pair.
 */
class NeuralDepth : public DeviceNodeCRTP<DeviceNode, NeuralDepth, NeuralDepthProperties> {
   public:
    ...
    std::shared_ptr<NeuralDepth> build(Node::Output& left, Node::Output& right, PresetMode presetMode = PresetMode::DEFAULT) {
    ...
    /**
     * Create NeuralDepth node. Note that this API is global and if used autocreated cameras can't be reused.
     ...
     */
    std::shared_ptr<NeuralDepth> build(bool autoCreateCameras,
   ...
   public:
    using MedianFilter = dai::NeuralDepthConfig::MedianFilter;
    ...
    /**
     * Input NeuralDepthConfig message with ability to modify parameters in runtime.
     */
    Input inputConfig{
        *this, {"inputConfig", DEFAULT_GROUP, DEFAULT_BLOCKING, DEFAULT_QUEUE_SIZE, {{{DatatypeEnum::NeuralDepthConfig, false}}}, DEFAULT_WAIT_FOR_MESSAGE}};
    ...
    /**
     * Outputs NeuralDepthConfig message that contains current stereo configuration.
     */
    Output outConfig{*this, {"outConfig", DEFAULT_GROUP, {{{DatatypeEnum::NeuralDepthConfig, false}}}}};
