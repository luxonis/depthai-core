...
class StereoDepth : public Node {
   ...
   public:
     ...
     */
    [[deprecated("Function call should be removed. Use setCalibrationData() under pipeline")]] void loadCalibrationFile(const std::string& path);

    /**
     * Specify calibration data as a vector of bytes
     * @param path Calibration data. If empty use EEPROM
     */
    [[deprecated("Function call should be removed. Use setCalibrationData() under pipeline")]] void loadCalibrationData(const std::vector<std::uint8_t>& data);
