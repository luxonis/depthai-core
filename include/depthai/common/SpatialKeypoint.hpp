...
struct SpatialKeypoint {
    ...
    explicit SpatialKeypoint(Point3f imageCoordinates, Point3f spatialCoordinates = Point3f{}, float conf = 0.f, uint32_t label = 0)
        : imageCoordinates(imageCoordinates), confidence(conf), label(label), spatialCoordinates(spatialCoordinates) {}

    explicit SpatialKeypoint(Point2f imageCoordinates, Point3f spatialCoordinates = Point3f{}, float conf = 0.f, uint32_t label = 0)
        : SpatialKeypoint(Point3f(imageCoordinates.x, imageCoordinates.y, 0.f), spatialCoordinates, conf, label) {}

    explicit SpatialKeypoint(float x, float y, float z, float sx, float sy, float sz, float conf = 0.f, uint32_t label = 0)
        : SpatialKeypoint(Point3f(x, y, z), Point3f{sx, sy, sz}, conf, label) {}

    DEPTHAI_SERIALIZE(dai::SpatialKeypoint, imageCoordinates, confidence, label, spatialCoordinates);
};
