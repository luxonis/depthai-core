...
def parse_args(argv):
    ...
    return log_path, junit_path, context, log_url, raw_log_url, log_map_path, step_number
...
def main() -> int:
    log_path, junit_path, context, _, _, _, _ = (
        parse_args(sys.argv)
    )
    if log_path is None or junit_path is None:
        print(
            "Usage: ctest_to_junit.py <log_path> <junit_output> [context] "
            "[--log-map <path>] [--log-url <url>] [--raw-log-url <url>] "
            "[--step-number <number>]"
        )
        return 0
