...
TEST(DynamicCalibration, DoWork) {
    ...
    auto command = std::make_shared<dai::DynamicCalibrationCommand>();
    command->calibrationCommand = dai::DynamicCalibrationCommand::Command::START_RECALIBRATION;
...
TEST(DynamicCalibration, DoWork_CallsInnerMockDclMethods) {
    ...
    dynCalib->properties.initialConfig.calibrationFrequency = 1;
    {
        ...
        auto command = std::make_shared<dai::DynamicCalibrationCommand>();
        command->calibrationCommand = dai::DynamicCalibrationCommand::Command::START_FORCE_CALIBRATION_QUALITY_CHECK;
    ...
    }
    {
        ...
        auto command = std::make_shared<dai::DynamicCalibrationCommand>();
        command->calibrationCommand = dai::DynamicCalibrationCommand::Command::START_CALIBRATION_QUALITY_CHECK;
        dynCalib->commandInput.send(command);
    ...
    }
    {
        ...
        auto err = dynCalib->doWork(previousLoadingTime);
        EXPECT_EQ(err, dai::node::DynamicCalibration::ErrorCode::CALIBRATION_FAILED);
    }
    {
        auto failedResult = dcl::Result<std::pair<std::shared_ptr<dcl::CameraCalibrationHandle>, std::shared_ptr<dcl::CameraCalibrationHandle>>>::error(1);
        // Expect checkCalibration to be called when forcing quality check
        EXPECT_CALL(*mockDclRaw, findNewCalibration(testing::_, testing::_, testing::_, dcl::PerformanceMode::SKIP_CHECKS))
            .Times(1)
            .WillOnce(testing::Return(failedResult));

        auto command = std::make_shared<dai::DynamicCalibrationCommand>();
        command->calibrationCommand = dai::DynamicCalibrationCommand::Command::START_FORCE_RECALIBRATION;
        dynCalib->commandInput.send(command);
    ...
    {
        auto command = std::make_shared<dai::DynamicCalibrationCommand>();
        command->calibrationCommand = dai::DynamicCalibrationCommand::Command::START_LOADING_IMAGES;
        dynCalib->commandInput.send(command);

        auto err1 = dynCalib->doWork(previousLoadingTime);
        EXPECT_EQ(err1, dai::node::DynamicCalibration::ErrorCode::OK);
        auto err2 = dynCalib->doWork(previousLoadingTime);
        EXPECT_EQ(err2, dai::node::DynamicCalibration::ErrorCode::EMPTY_IMAGE_QUEUE);

        command->calibrationCommand = dai::DynamicCalibrationCommand::Command::START_LOADING_IMAGES;
        dynCalib->commandInput.send(command);
        auto err3 = dynCalib->doWork(previousLoadingTime);
        EXPECT_EQ(err3, dai::node::DynamicCalibration::ErrorCode::OK);
    }
}
