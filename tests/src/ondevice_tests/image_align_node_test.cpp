...
TEST_CASE("Test ImageAlign generic path message to ImgFrame metadata") {
...
TEST_CASE("Test ImageAlign generic path ImgFrame to message metadata") {
...
TEST_CASE("Test ImageAlign generic path message to message metadata") {
...
TEST_CASE("Test ImageAlign generic path refreshes rectification metadata when ImgTransformation changes") {
    dai::Pipeline pipeline(false);
    auto align = pipeline.create<dai::node::ImageAlign>();
    align->setRunOnHost(true);
