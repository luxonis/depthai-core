...
TEST_CASE("FPS check") {
    ...
    for(const auto& [nodeId, nodeState] : state.nodeStates) {
        ...
        if(std::string(node->getName()) == "Camera") {
            ...
            if(!pipeline.isHolisticReplayEnabled()) REQUIRE(nodeState.outputStates.at("0").timing.fps == Catch::Approx(8.0).margin(2.0));
        }
        if(std::string(node->getName()) == "NeuralNetwork") {
            ...
            if(!pipeline.isHolisticReplayEnabled()) REQUIRE(nodeState.inputStates.at("in").timing.fps == Catch::Approx(8.0).margin(2.0));
            REQUIRE(nodeState.outputStates.at("passthrough").isValid());
            if(!pipeline.isHolisticReplayEnabled()) REQUIRE(nodeState.outputStates.at("passthrough").timing.fps == Catch::Approx(8.0).margin(2.0));
            REQUIRE(nodeState.outputStates.at("out").isValid());
            if(!pipeline.isHolisticReplayEnabled()) REQUIRE(nodeState.outputStates.at("out").timing.fps == Catch::Approx(8.0).margin(2.0));
        }
        if(std::string(node->getName()) == "StereoDepth") {
            ...
            if(!pipeline.isHolisticReplayEnabled()) REQUIRE(nodeState.inputStates.at("left").timing.fps == Catch::Approx(8.0).margin(2.0));
            REQUIRE(nodeState.inputStates.at("right").isValid());
            if(!pipeline.isHolisticReplayEnabled()) REQUIRE(nodeState.inputStates.at("right").timing.fps == Catch::Approx(8.0).margin(2.0));
            REQUIRE(nodeState.outputStates.at("depth").isValid());
            if(!pipeline.isHolisticReplayEnabled()) REQUIRE(nodeState.outputStates.at("depth").timing.fps == Catch::Approx(8.0).margin(2.0));
        }
        ...
        if(std::string(node->getName()) == "SpatialDetectionNetwork") {
            ...
            if(!pipeline.isHolisticReplayEnabled()) REQUIRE(nodeState.inputStates.at("inputDepth").timing.fps == Catch::Approx(8.0).margin(2.0));
            REQUIRE(nodeState.inputStates.at("inputDetections").isValid());
            if(!pipeline.isHolisticReplayEnabled()) REQUIRE(nodeState.inputStates.at("inputDetections").timing.fps == Catch::Approx(8.0).margin(2.0));
            REQUIRE(nodeState.inputStates.at("inputImg").isValid());
            if(!pipeline.isHolisticReplayEnabled()) REQUIRE(nodeState.inputStates.at("inputImg").timing.fps == Catch::Approx(8.0).margin(2.0));
            REQUIRE(nodeState.outputStates.at("passthroughDepth").isValid());
            if(!pipeline.isHolisticReplayEnabled()) REQUIRE(nodeState.outputStates.at("passthroughDepth").timing.fps == Catch::Approx(8.0).margin(2.0));
            REQUIRE(nodeState.outputStates.at("out").isValid());
            if(!pipeline.isHolisticReplayEnabled()) REQUIRE(nodeState.outputStates.at("out").timing.fps == Catch::Approx(8.0).margin(2.0));
        }
    }
