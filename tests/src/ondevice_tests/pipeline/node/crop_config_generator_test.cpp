...
GeneratorOutput runGenerator(const std::shared_ptr<dai::Buffer>& detections, const std::shared_ptr<dai::ImgFrame>& image) {
    ...
    REQUIRE_FALSE(generator->runOnHost());
    ...
    auto config = configQueue->get<dai::ImageManipConfig>(std::chrono::seconds(5), configTimedOut);
    auto outputImage = outputImageQueue->get<dai::ImgFrame>(std::chrono::seconds(5), imageTimedOut);
...
TEST_CASE("CropConfigGenerator generates paired crops from ImgDetections on device", "[CropConfigGenerator][ondevice]") {
    ...
    CHECK(output.image->getSequenceNum() == image->getSequenceNum());
    CHECK(output.image->getWidth() == image->getWidth());
    CHECK(output.image->getHeight() == image->getHeight());
}

TEST_CASE("CropConfigGenerator uses an explicitly set ImgDetection boundingBox on device", "[CropConfigGenerator][ondevice]") {
    auto detections = std::make_shared<dai::ImgDetections>();
...
TEST_CASE("CropConfigGenerator supports SpatialImgDetections on device", "[CropConfigGenerator][ondevice]") {
...
TEST_CASE("CropConfigGenerator uses an explicitly set SpatialImgDetection boundingBox on device", "[CropConfigGenerator][ondevice]") {
    auto detections = std::make_shared<dai::SpatialImgDetections>();
...
TEST_CASE("CropConfigGenerator supports Tracklets on device", "[CropConfigGenerator][ondevice]") {
