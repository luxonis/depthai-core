...
// so that their footprints on the ground overlap only partially. The origin of the whole dataset is CAM_A.
...
Dataset renderDataset(const std::vector<SyntheticCamera>& cameras) {
    ...
    for(const auto& camera : cameras) {
        ...
        extrinsics.toCameraSocket = ORIGIN;
...
TEST_CASE("Stitching projects a virtual rig onto the ground plane", "[Stitching]") {
    ...
    REQUIRE(transformation.getExtrinsics().toCameraSocket == ORIGIN);
...
TEST_CASE("Stitching rejects a changed planar origin after preparing the projection", "[Stitching]") {
    auto dataset = renderDataset(RIG);
