...
`Algorithm::AUTO` runs an internal backend-selection pass from the requested resolution, target FPS, device capabilities, and supported models:
...
Every backend has one (or several) row(s) in `BACKEND_PROFILES` in `src/pipeline/node/Depth.cpp`. Each row is `{algorithm, config, maxWidth, maxHeight, maxFps}`. Selection walks rows in this priority order and returns the first row whose algorithm is supported by the device, whose `maxFps ≥ targetFps × 0.85`, and (when a resolution is provided) whose resolution rule passes:
...
3. **StereoDepth** — 3 quality-ordered presets (`DEFAULT` @ 30 FPS → `FAST_DENSITY` / `FAST_ACCURACY` @ 60 FPS), all capped at 1280×1280.
4. **GPUStereo** — 3 rows: **2592×1944 @ 5 FPS**, **1280×800 @ 30 FPS**, **640×400 @ 55 FPS**.
