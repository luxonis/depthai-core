...
print("PYTHONPATH includes shared:", SHARED_DIR)
...
def parse_args():
    ...
    parser.add_argument("-brd", "--board", default=None, type=str,
                        help="BW1097, BW1098OBC - Board type from ../shared/depthai_boards/boards (not case-sensitive). "
    ...
    parser.add_argument('-fps', '--framerate', type=float, default=5,
...
class Main:
    ...
    def __init__(self):
        ...
        if self.args.board:
            ...
            if not board_path.exists():
                board_path = (Path(__file__).parent.parent / 'shared/depthai_boards/boards' / self.args.board.upper()).with_suffix('.json').resolve()
                if not board_path.exists():
                    raise ValueError(
            ...
            if not board_path.exists():
                board_path = (Path(__file__).parent.parent / 'shared/depthai_boards/boards' / self.board_name.upper()).with_suffix('.json').resolve()
                if not board_path.exists():
                    raise ValueError(
    ...
    def calibrate(self):
        ...
        dest_path = str(Path('shared').absolute())
