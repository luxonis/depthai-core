...
RUN git clone --branch 1.9.2 https://github.com/flann-lib/flann.git \
    && cd flann \
    && cmake -S $(pwd) -Bbuild -D'CMAKE_BUILD_TYPE=Release' \
...
RUN cd pcl \
    && cmake -S. -Bbuild -D'WITH_VTK=OFF' -D'WITH_PNG=OFF' -D'PCL_SHARED_LIBS=ON' -D'PCL_ENABLE_SSE=OFF' -D'BUILD_TESTS=OFF' -D'BUILD_examples=OFF'  -D'WITH_OPENGL=OFF' -D'CMAKE_C_STANDARD=99' -D'CMAKE_POSITION_INDEPENDENT_CODE=ON' -D'CMAKE_CXX_FLAGS=-fopenmp -O3 -fno-omit-frame-pointer -mno-omit-leaf-frame-pointer  -flto=auto -ffat-lto-objects -fstack-protector-strong -fstack-clash-protection -Wformat -Werror=format-security -fcf-protection' -D'CMAKE_C_FLAGS=-fopenmp'  -D'CMAKE_BUILD_TYPE=Release' -D'CMAKE_SHARED_LINKER_FLAGS=-Wl,-Bsymbolic-functions -flto=auto -ffat-lto-objects -Wl,-z,relro -Wl,--as-needed' -D'CMAKE_EXE_LINKER_FLAGS=-Wl,-Bsymbolic-functions -flto=auto -ffat-lto-objects -Wl,-z,relro' -D'PCL_INDEX_SIGNED=true' -D'PCL_INDEX_SIZE=32' -D'PCL_ONLY_CORE_POINT_TYPES=ON' \
    && cmake --build build --target install --parallel 8
