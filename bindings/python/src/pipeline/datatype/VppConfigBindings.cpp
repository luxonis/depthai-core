...
void bind_vppconfig(pybind11::module& m, void* pCallstack) {
    ...
    // 2. Bind the Enum INSIDE vppConfig instead of m
    py::enum_<VppConfig::PatchColoringType>(vppConfig, "PatchColoringType", DOC(dai, VppConfig, PatchColoringType))
    ...
    // 3. Bind InjectionParameters (can stay on m or be moved to vppConfig depending on preference)
    py::class_<VppConfig::InjectionParameters>(vppConfig, "InjectionParameters", DOC(dai, VppConfig, InjectionParameters))
        .def(py::init<>())
    ...
    // 5. Define VppConfig methods and properties
