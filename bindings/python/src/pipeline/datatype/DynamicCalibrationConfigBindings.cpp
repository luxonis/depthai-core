...
void bind_dynamic_calibration_config(py::module& m, void* pCallstack) {
    ...
    // Command types
    py::class_<CalibrateCommand, std::shared_ptr<CalibrateCommand>>(m, "CalibrateCommand")
        .def(py::init<bool>(), py::arg("force") = false)
        .def_readwrite("force", &CalibrateCommand::force);

    py::class_<CalibrationQualityCommand, std::shared_ptr<CalibrationQualityCommand>>(m, "CalibrationQualityCommand")
        .def(py::init<bool>(), py::arg("force") = false)
        .def_readwrite("force", &CalibrationQualityCommand::force);

    py::class_<StartCalibrationCommand, std::shared_ptr<StartCalibrationCommand>>(m, "StartCalibrationCommand")
    ...
    py::class_<StopCalibrationCommand, std::shared_ptr<StopCalibrationCommand>>(m, "StopCalibrationCommand")
        .def(py::init<>())
        .def_readwrite("nothing", &StopCalibrationCommand::nothing);

    py::class_<LoadImageCommand, std::shared_ptr<LoadImageCommand>>(m, "LoadImageCommand").def(py::init<>());

    py::class_<ApplyCalibrationCommand, std::shared_ptr<ApplyCalibrationCommand>>(m, "ApplyCalibrationCommand")
        .def(py::init<>())
        .def(py::init<const dai::CalibrationHandler&>(), py::arg("calibration"))
        .def_readwrite("calibration", &ApplyCalibrationCommand::calibration);

    py::class_<ResetDataCommand, std::shared_ptr<ResetDataCommand>>(m, "ResetDataCommand").def(py::init<>());

    py::class_<SetPerformanceModeCommand, std::shared_ptr<SetPerformanceModeCommand>>(m, "SetPerformanceModeCommand")
