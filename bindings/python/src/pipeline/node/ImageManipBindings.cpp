...
void bind_imagemanip(pybind11::module& m, void* pCallstack) {
    ...
    imageManip.def_readonly("inputConfig", &ImageManip::inputConfig, DOC(dai, node, ImageManip, inputConfig))
        ...
        .def(
            ...
            py::arg("wait"),
            DOC(dai, node, ImageManip, setWaitForConfigInput))
        ...
        .def("setWarpMesh",
             py::overload_cast<const std::vector<std::pair<float, float>>&, int, int>(&ImageManip::setWarpMesh),
             DOC(dai, node, ImageManip, setWarpMesh));
}
