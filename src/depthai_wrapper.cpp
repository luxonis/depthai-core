...
DepthAI::DepthAI(std::string usb_device, std::string config_file, bool usb2_mode) :
...
void DepthAI::set_resolution()
{
    ...
    if (config_json_.contains("camera"))
    {
        ...
        if (camera_conf_obj.contains("mono"))
        {
            ...
            mono_width_ = width_map_[mono_height_];
        }
        else
        {
            mono_height_ = 720;
    ...
    if (config_json_.contains("streams"))
    {
        for (auto it : config_json_.at("streams"))
        {
            ...
            if (it.is_string())
            {
                ...
                {
                    std::shared_ptr<cv::Mat> img =
                        std::make_shared<cv::Mat>(mono_height_, mono_width_, CV_8UC3);
                    image_streams_.push_back(img);
                }
            }
            else
...
// each time get frames is called ?
