...
std::array<float, 3> opencvUndistortPoint(dai::Point2f px, const dai::ImgTransformation& transformation) {
    ...
    switch(distortionModel) {
        ...
        case dai::CameraModel::Equirectangular:
        default: {
            const std::array<float, 3> pxHomogeneous = {px.x, px.y, 1.0f};
            const std::array<float, 3> pxSensor = dai::matrix::matVecMul(transformation.getSourceIntrinsicMatrixInv(), pxHomogeneous);
            const auto undistortedRay = undistortPoint(pxSensor, distortionModel, distortionCoeffsVec);
            return {undistortedRay[0] / undistortedRay[2], undistortedRay[1] / undistortedRay[2], 1.0f};
        }
    }
...
dai::Point2f opencvDistortRay(const std::array<float, 3> ray, const dai::ImgTransformation& transformation) {
    ...
    switch(distortionModel) {
        ...
        default: {
            ...
            const std::array<float, 3> distortedHomogeneous = {distorted[0] / distorted[2], distorted[1] / distorted[2], 1.0f};
            const std::array<float, 3> projectedHomogeneous = dai::matrix::matVecMul(transformation.getSourceIntrinsicMatrix(), distortedHomogeneous);
            return {projectedHomogeneous[0] / projectedHomogeneous[2], projectedHomogeneous[1] / projectedHomogeneous[2]};
        }
    }
