...
DynamicCalibration::ErrorCode DynamicCalibration::runCalibration(const CalibrationHandler& currentCalibHandler, const bool force) {
    dcl::PerformanceMode performanceMode = force ? dcl::PerformanceMode::SKIP_CHECKS : properties.initialConfig.performanceMode;
    ...
    newCalibrationHandler = std::make_shared<CalibrationHandler>(currentCalibHandler);
    // clang-format off
...
DynamicCalibration::ErrorCode DynamicCalibration::runLoadImage() {
    auto inSyncGroup = syncInput.tryGet<dai::MessageGroup>();
    if(!inSyncGroup) {
...
DynamicCalibration::ErrorCode DynamicCalibration::initializePipeline(const std::shared_ptr<dai::Device> daiDevice) {
    ...
    calibrationHandler = std::make_shared<dai::CalibrationHandler>(daiDevice->getCalibration());
    initialCalibrationHandler = calibrationHandler;
    previousCalibrationHandler = calibrationHandler;
...
DynamicCalibration::ErrorCode DynamicCalibration::doWork(std::chrono::steady_clock::time_point& previousLoadingTime) {
    auto calibrationCommandInput = commandInput.tryGet<DynamicCalibrationCommand>();

    if(!calibrationCommandInput) {
        if(!loadingImages) {
            return ErrorCode::OK;
        }
        ...
        if(elapsed.count() > properties.initialConfig.loadImageFrequency) {
            loadError = runLoadImage();
            if(loadError == ErrorCode::OK) {
                auto coverageResult = computeCoverage();
    ...
    auto calibrationCommand = calibrationCommandInput->calibrationCommand;

    switch(calibrationCommand) {
        case DynamicCalibrationCommand::Command::START_FORCE_CALIBRATION_QUALITY_CHECK:
            return runQualityCheck(true);
        case DynamicCalibrationCommand::Command::START_CALIBRATION_QUALITY_CHECK:
            return runQualityCheck();
        case DynamicCalibrationCommand::Command::START_RECALIBRATION:
            return runCalibration(*calibrationHandler);
        case DynamicCalibrationCommand::Command::START_FORCE_RECALIBRATION:
            return runCalibration(*calibrationHandler, true);
        case DynamicCalibrationCommand::Command::START_LOADING_IMAGES:
            loadingImages = true;
            return ErrorCode::OK;
        case DynamicCalibrationCommand::Command::STOP_LOADING_IMAGES:
            loadingImages = false;
            return ErrorCode::OK;
        case DynamicCalibrationCommand::Command::APPLY_NEW_CALIBRATION:
            if(newCalibrationHandler) {
                setCalibration(*newCalibrationHandler);
                previousCalibrationHandler = calibrationHandler;
                calibrationHandler = newCalibrationHandler;
                newCalibrationHandler = nullptr;
                return ErrorCode::OK;
            }
            return ErrorCode::CALIBRATION_DOES_NOT_EXIST;
        case DynamicCalibrationCommand::Command::APPLY_PREVIOUS_CALIBRATION:
            if(previousCalibrationHandler) {
                setCalibration(*previousCalibrationHandler);
                newCalibrationHandler = calibrationHandler;
                calibrationHandler = previousCalibrationHandler;
                previousCalibrationHandler = nullptr;
                return ErrorCode::OK;
            }
            return ErrorCode::CALIBRATION_DOES_NOT_EXIST;
        case DynamicCalibrationCommand::Command::APPLY_INITIAL_CALIBRATION:
            if(initialCalibrationHandler) {
                setCalibration(*initialCalibrationHandler);
                calibrationHandler = initialCalibrationHandler;
                return ErrorCode::OK;
            }
            return ErrorCode::CALIBRATION_DOES_NOT_EXIST;
        default:
    ...
    std::chrono::steady_clock::time_point& previousLoadingTime)
{
    ...
    if(elapsedCalibration.count() > properties.initialConfig.calibrationFrequency) {
        auto calibrationError = runCalibration(device->getCalibration());
	if (calibrationError == DynamicCalibration::ErrorCode::OK) {
	    setCalibration(*newCalibrationHandler);
	}
    ...
    if (elapsedLoading.count() > properties.initialConfig.loadImageFrequency) {
...
void DynamicCalibration::run() {
    ...
    auto previousLoadingTimeFloat = std::chrono::steady_clock::now() + std::chrono::duration<float>(properties.initialConfig.calibrationFrequency);
