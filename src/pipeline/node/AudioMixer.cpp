...
void AudioMixer::linkSourceToSink(std::string sourceName, std::string sinkName) {
    if(audioSources.find(sourceName) != audioSources.end() && audioSinks.find(sinkName) != audioSinks.end()) {
...
std::shared_ptr<AudioFrame> AudioMixer::AudioMixerSink::mix() {
    ...
    for(auto [source, data] : sourceData) {
        if(data->frames.front()->getBitrate() != bitrate || data->frames.front()->getFormat() != format || data->frames.front()->getChannels() != channels) {
        ...
        size_t count = largestSize > data->frames.front()->getData().size() ? data->frames.front()->getData().size() : largestSize;
