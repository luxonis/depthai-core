...
HostPipeline::HostPipeline() : _data_queue_lf(c_data_queue_size) {}

void HostPipeline::onNewData(const StreamInfo& info, const StreamData& data) {
    Timer t;

    // std::cout << "--- new data from " << info.name << " , size: " << data.size << "\n";
    bool keep_frame = _public_stream_names.empty() || (_public_stream_names.find(info.getName()) != _public_stream_names.end());
    if(!keep_frame) {
    ...
    std::shared_ptr<HostDataPacket> host_data(new HostDataPacket(data.size, data.data, info.name, info.getDimensionsForSize(data.size), info.elem_size));
...
std::list<std::shared_ptr<HostDataPacket>> HostPipeline::getAvailableDataPackets(bool blocking) {
    ...
    std::function<void(std::shared_ptr<HostDataPacket>&)> functor = [&result](std::shared_ptr<HostDataPacket>& data) { result.push_back(data); };
