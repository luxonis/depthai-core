            ...
            if(forceRequiredOpenVINOVersion){
                ...
                if(!version){
                    ...
                    if(!OpenVINO::areVersionsBlobCompatible(*version, *requiredVersion)){
                        ...
                        std::string err = fmt::format("Pipeline - OpenVINO version required by '{}' node (id: {}), isn't compatible with '{}' node (id: {})", lastNodeNameWithRequiredVersion, lastNodeIdWithRequiredVersion, node->getName(), node->id);
                        throw std::logic_error(err.c_str());
                    }
                }
            }
