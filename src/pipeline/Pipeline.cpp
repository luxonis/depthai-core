...
void PipelineImpl::serialize(
    ...
    // Set schema
    schema = getDevicePipelineSchema(type, true, std::move(deviceId));
...
void PipelineImpl::setMultiDeviceCalibration(const CalibrationHandler& multiDeviceCalibrationHandler) {
    std::lock_guard<std::mutex> lock(calibMtx);
    multiDeviceCalibration = std::make_shared<CalibrationHandler>(multiDeviceCalibrationHandler);
}

std::shared_ptr<const CalibrationHandler> PipelineImpl::getMultiDeviceCalibration() const {
    std::lock_guard<std::mutex> lock(calibMtx);
...
void PipelineImpl::start() {
    ...
    // Add pointer to the pipeline to the devices before device-side startup can emit telemetry.
    if(!devices.empty()) {
    ...
    if(!isHostOnly()) {
        ...
        for(const auto& device : devices) {
            device->startPipeline(Pipeline(shared_from_this()));
        }
    }
...
void PipelineImpl::resetConnections() {
    ...
    for(auto node : getAllNodes()) {
        ...
        if(tmp2) {
            ...
            auto device = it != bridgeHostDevices.end() ? it->second : defaultDevice;
            if(device == nullptr || device->getConnection() == nullptr) throw std::runtime_error("Connection lost");
            tmp2->setConnection(device->getConnection());
        }
    }
