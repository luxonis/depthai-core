...
class HumanPoseEstimationVisualizer: public dai::NodeCRTP < dai::node::HostNode, HumanPoseEstimationVisualizer > {
    ...
    // ! life of me figure out how to do it
        ...
        std::vector < std::vector < std::tuple < cv::Point,
        float,
        int > > > DETECTED_KEYPOINTS;
        std::vector < std::tuple < cv::Point,
        float > > KEYPOINTS_LIST;
        ...
        // Returns a vector of keypoints (point and confidence)
        // Is run cca. 19 times, once for every thing we are meassuring
        ...
        std::pair < std::vector < std::vector < std::tuple < int, int, float > > > , std::vector < int > >
            getValidPairs(  const xt::xarray < float > & outputs, int w, int h,
