...
// Add blue square to the image
class AddBlueSquare : public dai::NodeCRTP<dai::node::HostNode, AddBlueSquare> {
   public:
    Input& input = inputs["in"];

    std::shared_ptr<AddBlueSquare> build(Output& out) {
        out.link(input);
        return std::static_pointer_cast<AddBlueSquare>(this->shared_from_this());
    }

    std::shared_ptr<dai::Buffer> processGroup(std::shared_ptr<dai::MessageGroup> in) override {
        auto inValue = in->get<dai::ImgFrame>("in");
        auto inFrame = inValue->getCvFrame();
        cv::rectangle(inFrame, cv::Point(50, 50), cv::Point(100, 100), cv::Scalar(255, 0, 0), -1);
        ...
        std::vector<std::vector<int> >getPersonwiseKeypoints(
            ...
            for (int keypointPairIndedx = 0; keypointPairIndedx < MAPIDX.size(); keypointPairIndedx++){
                ...
                for(int pairIndex = 0; pairIndex < validPairs[keypointPairIndedx].size(); pairIndex++){
                    ...
                    for(int personwiseKeypointsIndex = 0; personwiseKeypoints.size(); personwiseKeypointsIndex++){
...
int main() {
    ...
    auto resize = pipeline.create<Resize>()->build(camRgb->video, 456, 256);
