...
with dai.Pipeline(True) as pipeline:
    ...
    while True:
        ...
        if key == ord("q"):
            ...
            if thermalConf.imageParams.brightnessLevel <= THERMAL_IMAGE_BRIGHTNESS_STEP:
                thermalConf.imageParams.brightnessLevel = THERMAL_IMAGE_BRIGHTNESS_STEP
            thermalConf.imageParams.brightnessLevel -= THERMAL_IMAGE_BRIGHTNESS_STEP
            print("Set image brightness level to ", thermalConf.imageParams.brightnessLevel)
            changed = True
        elif key == ord("m"):
            if thermalConf.imageParams.brightnessLevel is None:
                thermalConf.imageParams.brightnessLevel = 0
            if thermalConf.imageParams.brightnessLevel >= 255:
                thermalConf.imageParams.brightnessLevel = 255 - THERMAL_IMAGE_BRIGHTNESS_STEP
            thermalConf.imageParams.brightnessLevel += THERMAL_IMAGE_BRIGHTNESS_STEP
            print("Set image brightness level to ", thermalConf.imageParams.brightnessLevel)
            changed = True
        elif key == ord("a"):
            ...
            print("Set auto shutter to", "on." if thermalConf.ffcParams.autoFFC else "off.")
            changed = True
        elif key == ord("s"):
            if thermalConf.ffcParams.closeManualShutter is None:
                thermalConf.ffcParams.closeManualShutter = False
            thermalConf.ffcParams.closeManualShutter = not thermalConf.ffcParams.closeManualShutter
            print("Closing" if thermalConf.ffcParams.closeManualShutter else "Opening", "manual shutter.")
            changed = True
