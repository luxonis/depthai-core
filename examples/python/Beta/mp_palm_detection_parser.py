...
def main() -> None:
    with dai.Pipeline() as pipeline:
        modelSlug = "luxonis/mediapipe-palm-detection:192x192"
        ...
        parserNode = pipeline.create(dai.beta.node.MPPalmDetectionParser).build(
            neuralNetwork.out,
            modelArchive,
        )
        ...
        while pipeline.isRunning():
            ...
            cv2.imshow("MPPalmDetectionParser", frame)
            key = cv2.waitKey(1)
            if key == ord("q"):
                break
            if key == ord("+"):
