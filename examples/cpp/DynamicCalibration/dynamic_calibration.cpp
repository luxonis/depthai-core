...
int main() {
    ...
    while(pipeline.isRunning()) {
        ...
        const auto calibrationResult = calibrationOutput->get<dai::DynamicCalibrationResult>();
        const auto& calibrationData = calibrationResult->calibrationData;
        if (calibrationData.has_value())
        {
            const auto calibrationHandler = calibrationData.value();
            commandInput->send(std::make_shared<dai::ApplyCalibrationCommand>(calibrationHandler.newCalibration));
            std::cout << "Successfully recalibrated" << std::endl;
        }
        else
        {
            std::cout << calibrationResult->info << std::endl;
        }
    }
