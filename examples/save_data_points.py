...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="CAM_A",
        choices=["CAM_A", "CAM_B", "CAM_C", "CAM_D", "CAM_E", "CAM_B_R", "CAM_C_R"],
    ...
    try:
        with dai.Pipeline() as pipeline:
            ...
            camera_d_output = camera_a.requestOutput(
                (1080, 400),
                fps=fps,
                resizeMode=dai.ImgResizeMode.STRETCH,
                enableUndistortion=not enable_undistortion,
            )
            camera_e_output = camera_a.requestOutput(
                size, type=dai.ImgFrame.Type.NV12, fps=fps, resizeMode=dai.ImgResizeMode.LETTERBOX, enableUndistortion=enable_undistortion
            )

            link_record_node(pipeline, camera_a_output, f"{output_path}_CAM_A")
            link_record_node(pipeline, camera_d_output, f"{output_path}_CAM_D", encode=False)
            link_record_node(pipeline, camera_e_output, f"{output_path}_CAM_E")
            ...
            if save_depth:
                ...
                camera_b_rectified_output = stereo.rectifiedLeft
                camera_c_rectified_output = stereo.rectifiedRight

                link_record_node(pipeline, camera_b_rectified_output, f"{output_path}_CAM_B_R", encode=False)
                link_record_node(pipeline, camera_c_rectified_output, f"{output_path}_CAM_C_R", encode=False)
                link_record_metadata_node(pipeline, stereo.depth, f"{output_path}_DEPTH")
                link_record_node(pipeline, camera_c_output, f"{output_path}_CAM_C", encode=False)
                link_record_node(pipeline, camera_b_output, f"{output_path}_CAM_B", encode=False)

                image_align = pipeline.create(dai.node.ImageAlign)
                reference_camera_output = {
                    "CAM_A": camera_a_output,
                    "CAM_B": camera_b_output,
                    "CAM_C": camera_c_output,
                    "CAM_B_R": camera_b_rectified_output,
                    "CAM_C_R": camera_c_rectified_output,
                    "CAM_D": camera_d_output,
                    "CAM_E": camera_e_output,
                }[reference_camera]
                ...
                link_record_metadata_node(pipeline, depth_output, f"{output_path}_DEPTH_ALIGNED")
                depth_output_queue = depth_output.createOutputQueue()
            else:
                reference_camera_output = {
                    "CAM_A": camera_a_output,
                    "CAM_D": camera_d_output,
            ...
            reference_output_queue = reference_camera_output.createOutputQueue()
            april_tag_output_queue = april_tag_node.out.createOutputQueue()
            ...
            while pipeline.isRunning():
                in_rgb_message = reference_output_queue.get()
                in_april_message = april_tag_output_queue.get()

                assert isinstance(in_april_message, dai.AprilTags)
                assert isinstance(in_rgb_message, dai.ImgFrame)

                in_depth_message = None
                if save_depth and depth_output_queue is not None:
                    in_depth_message = depth_output_queue.get()
                    assert isinstance(in_depth_message, dai.ImgFrame)
                    depth_frame = in_depth_message.getFrame()
                    # Use intrinsics from the aligned depth frame itself.
                    intrinsics = in_depth_message.getTransformation().getIntrinsicMatrix()
                    data_points = april_tag_to_spatial_points(in_april_message, depth_frame, intrinsics)
                    points_field = "points_spatial_mm"
                else:
                    frame_width = in_rgb_message.getWidth()
                    frame_height = in_rgb_message.getHeight()
                    data_points = april_tag_to_points(in_april_message, frame_width, frame_height)
                    points_field = "points_normalized"

                if visualize:
                    rgb_frame = in_rgb_message.getCvFrame()
